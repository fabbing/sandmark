...
case "$CONFIG_VARIANT" in
    *5.1.0*)
        ...
        jq '{wrappers : .wrappers, benchmarks: [.benchmarks | .[] | select( .name as $name | ["irmin_replay", "cpdf", "frama-c", "js_of_ocaml", "graph500_kernel1", "graph500_kernel1_multicore"] | index($name) | not )]}' $RUN_CONFIG_JSON
          >$RUN_CONFIG_JSON.tmp;
